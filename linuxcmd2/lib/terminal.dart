...
class _TerminalState extends State<Terminal> {
 String cmd;
  ...
  setState(() {
    ...
    k=k+1;
    await fsconnect.collection("streambuild").add({
      'id' : k,
      'command' : cmd,
       'output' : webdata,
     });
  ...
  /*getoutputlive() async{
    await for (var ss in fsconnect.collection("streambuild").snapshots()){
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            Container(
                              ...
                              StreamBuilder<QuerySnapshot>(                             //get output from stream
                                builder: (context, snapshot){
                                  var output = snapshot.data.docs;
                                  //print(output);
                                  List<Widget> y = [];
                                  for (var d in output) {
                                    var op = d.data()['output'];
                                    var cmd = d.data()['command'];
                                    var txt = Text("$op",style: TextStyle(fontSize: 15,color: Colors.white70,), );
                                    y.add(txt);
                                  }
                                 ...
                                 child: SingleChildScrollView(
                                     ...
                                     crossAxisAlignment: CrossAxisAlignment.start,
                                     children: y,
                                       //Text( z ?? ""  , style: TextStyle(fontSize: 15,color: Colors.white70,),),
                              ...
                              stream: fsconnect.collection("streambuild").orderBy("id", descending: true).snapshots(),
                              ),
